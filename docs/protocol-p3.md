...
| Offset | Length | Field    | エンディアン     | 備考 |
|-------:|-------:|----------|------------------|------|
| 0      | 1      | SOR      | -                | 必ず `0x8E` |
| 1      | 1      | Version  | -                | プロトコルバージョン |
| 2      | 2      | Length   | **little-endian**| ペイロード長(範囲は要検証、§9 参照) |
...
```
+-------------+--------+---------------------+
| Field ID    | Length | Value (Length bytes)|
|   1 byte    | 1 byte |   little-endian     |
+-------------+--------+---------------------+
```

- **Field ID**(1 byte): `0x81` などのレコード属性 ID。TOR ごとの定義(§7)を引く。`GENERAL`(§7.1)に該当する場合は共通フィールド。
- **Length**(1 byte): 値の長さ(バイト数)。`uint8` として読む。
- **Value**(Length bytes): リトルエンディアン。文字列か数値かは Field ID により決まる(§9 で精緻化予定)。

**終端条件**: ボディ走査中に `0x8F` を読んだら TLV 列の終端とみなす(参考実装と同等)。

> ⚠ **参考実装の不具合に注意**: `decoder.py:_decode_record` で Length バイトを `int(codecs.encode(byte, 'hex'))` で取得しており、`0x10` が **10**(本来は 16)として解釈される。本仕様では Length は **`uint8` として直接読む**(`bytes[1]`)。
...
| `0x0003` | `VERSION`         | §7.5           |
| `0x0004` | `RESEND`          | §7.6           |
| `0x0005` | `CLEAR_PASSING`   | (無)           |
| `0x0013` | `SERVER_SETTINGS` | (無)           |
| `0x0015` | `SESSION`         | (無)           |
| `0x0016` | `NETWORK_SETTINGS`| (無)           |
| `0x0018` | `WATCHDOG`        | (無)           |
| `0x0020` | `PING`            | (無)           |
| `0x0024` | `GET_TIME`        | §7.7           |
| `0x0028` | `GENERAL_SETTINGS`| (無)           |
| `0x002D` | `SIGNALS`         | (無)           |
| `0x002F` | `LOOP_TRIGGER`    | (無)           |
| `0x0030` | `GPS_INFO`        | (無)           |
| `0x0045` | `FIRST_CONTACT`   | (無)           |
| `0x004A` | `TIMELINE`        | (無)           |
| `0xFFFF` | `ERROR`           | §7.8           |
...
| # | 項目 | 影響範囲 |
|---|------|----------|
| 1 | ヘッダ `Length` フィールドの示す範囲(SOR 込み? エスケープ前/後?) | 受信バッファ管理 |
...
| `decoder.py:_decode_record` | Length を `int(hex(byte))` として **10 進文字列パース**しており、`0x10` 以上で誤読。**`uint8` で直読**すること。 |
