...
### 2.2 β-2 現地(AMB 疎通)専用テンプレート

`docs/test-strategy.md` §6.3 で「現地、AMB 疎通のみ、~30 分」と定義した最小スコープ。`/admin` や Soak は β-1 で済んでいるので **本セッションは AMB との実フレーム到達 + 1 周走行確認だけ**。当日操作員が見るチェックリストは [`docs/field-test-checklist-beta2.md`](field-test-checklist-beta2.md)。

```markdown
## YYYY-MM-DD β-2 (Windows ___ / iOS ___ Safari / 会場 業務 WiFi)

### 環境
- ゲートウェイ: vX.Y.Z (commit `<short-sha>`、USB 配布物 `dist\AMB_RC_Lap_Timer\` 一式)
- ホスト PC: 持ち込みノート(Windows ___ build ___、自宅で SmartScreen / Firewall 承認済み)
- クライアント: iPhone ___(Safari ___)
- ネットワーク: 会場 業務 WiFi(SSID 伏字、5GHz/2.4GHz どちらか)、AMB は __:__:__:__:__:__ MAC の機器
- AMB エンドポイント: `<IP>:5403`(前回採取と同じか確認)
- 実施者: ___

### 実施シナリオと結果
| シナリオ | 結果 | メモ |
|---|---|---|
| AMB 疎通(`ping <AMB IP>`) | ✅/⚠/❌ | 平均 __ms、ロス __/N |
| TCP 接続(`gateway --upstream <AMB>:5403` でログに `upstream connected`) | | 接続まで __秒 |
| 実フレーム到達(/healthz `{"upstream":"connected"}`) | | 自分のトランスポンダーで N 周走行 |
| ラップ表示(iPhone Safari)| | 大型表示 + Lap List + ★ ハイライトが出るか |
| ラップ読み上げ(iPhone Safari)| | 「N 秒 X Y」形式で発話されるか |

### 観察
- ___(数値・気付き・予想と違ったこと)
- AMB IP / ポートが前回と異なっていた場合はその記録(`/admin` で更新したか、config.json 直編集か)

### 派生 Issue / PR
- なし(or #___)

### 次のアクション
- 問題なければ **v0.1.0 リリースは β-2 通過の証跡として確定**
- 派生 Issue が出たら β-2-2(再訪 or 自宅 mock 補修)で再検証
```
