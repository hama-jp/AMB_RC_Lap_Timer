...
> Status: **Draft v0.1.6**(実装フェーズ #1-#8 + α-1 自走分 + β-1 自宅 dry-run + 修正再検証まで完了。残るは #100 Sleep/Wake / β-2 現地 / #9 リリース)
...
[実装フェーズ 続き]
  ...
  ✅ #8  設定 WebUI(/admin)+ 認証 + 監査ログ + USB 移植性 fix             (PR #88 #92 #93 #94 #105)
  ✅ ★   Field Test β-1 自宅 dry-run                                         (PR #96 + #102 + #106)
  ⌛ #100 iPhone Safari Sleep/Wake 後の WS 再接続 — β-2 唯一のブロッカー
  ⏳ ★   Field Test β-2(現地、AMB 疎通確認のみ)
  ⏳ #9  リリース自動化(v0.1.0、ZIP 配布)

[フォローアップ]
  ✅ #40 real.Source Close 後 Read の契約違反                                (PR #80)
  ✅ #41 real.Source.buf サイズ表記揺れ                                      (PR #89)
  ✅ #98 Speech digit-by-digit 形式                                          (PR #103)
  ✅ #99 Defender Firewall 復旧 docs                                         (PR #104)
  ✅ #101 /admin POST 絶対パス焼き込み修正(USB 移植性)                     (PR #105)
...
### ★ Field Test β-1(自宅 dry-run、AMB 以外)  ✅(PR #96 + #102 + #106)
- **Soak 1h(PR #96)**: ws_mb_delta=+4.8% / handle_delta=+0.2% / reconnects=0、線形外挿で 8h まで耐える根拠
- **9 シナリオ本番(PR #102)**: Smoke / Speech / Multi-client / Sleep-Wake / WiFi drop / `/admin` E2E / USB 起動 / USB 抜き挿し / Soak。✅ 4 / ⚠ 3 / ❌ 1 で派生 Issue 4 件起票
- **修正後の再検証(PR #106)**: B-2 / B-7 / B-8 すべて ✅ 化、β-2 ブロッカーは #100 のみに収斂
- 詳細記録: `docs/field-test-log.md` §3 の 3 セッション

### ⌛ #100 iPhone Safari Sleep/Wake 後の WS 再接続(β-2 ブロッカー)
- 症状: スマホ画面ロック → 1〜2 分 → 解除で「PASSING を待機中」のまま固まる
- 仮説: iOS suspension で WS の `onclose` が発火せず、`wsClient` の reconnect が trigger されない
- 修正方向: `wsClient.ts` に visibilitychange + heartbeat 監視を追加、または `visible` 遷移時に `reconnect()` を 1 回叩く
- 着手: Windows エージェントが担当(2026-05-07 時点)
...
| ~~`real.Source.buf` 4096 vs 仕様 10240~~ | ✅ #41 | 案 A 採用、`recvBufSize=10240` 定数化(PR #89) |
| ~~admin 設計詰め~~ | ✅ #79 | docs/architecture.md §3.5 確定(PR #85) |
| ~~admin API + バリデーション~~ | ✅ #82 | atomic rename + 反映タイミング分類(PR #92) |
| ~~admin 認証 + 監査ログ~~ | ✅ #83 | one-time passphrase + cookie(PR #88)、constant-time 比較(PR #93) |
| ~~admin フロント UI~~ | ✅ #84 | HashRouter + 設定フォーム(PR #94) |
| ~~Speech 発話崩壊~~ | ✅ #98 | digit-by-digit「19秒4 9」形式(PR #103) |
| ~~Firewall 復旧 docs~~ | ✅ #99 | `packaging/README.txt` に 2 段階手順(PR #104) |
| ~~/admin POST 絶対パス焼き込み~~ | ✅ #101 | Raw / Resolved 分離(PR #105) |
| ~~β-1 自宅 dry-run~~ | ✅ #97 | Soak 1h + 9 シナリオ + 修正再検証(PR #96 / #102 / #106) |

### 進行中・未解決

| 議題 | Issue | 解消フェーズ | 備考 |
|---|---|---|---|
| iPhone Safari Sleep/Wake 後の再接続 | #100 | β-2 ブロッカー、Windows エージェント着手中 | visibilitychange + heartbeat 監視 |
| 上流接続状態の UI 表現(WS テキスト frame サーバ側) | #28 | 任意 | クライアント側は `/healthz` polling で代替済み(PR #63) |
...
## 5. 改訂履歴
- v0.1.6 (2026-05-07): §2 全体マップ・§3 を Field Test β-1 自宅 dry-run 完了で更新(Soak 1h PR #96 + 9 シナリオ本番 PR #102 + 修正再検証 PR #106)。§4 解決済みを #41 / #79 / #82-#84 / #98 / #99 / #101 / #97 まで取り込み、未解決は **#100(β-2 ブロッカー)/ #28 / #30 / #47** の 4 件のみに収斂。残りの開発は #100 修正 → β-2 現地(AMB 疎通)→ #9 リリースの 3 段階。
