...
> Status: **Draft v0.1.4**(実装フェーズ #1-#7 完了 + Field Test α-1 自走分実施済み、★ Field Test α 人手分待ち / #8 設計詰め中)
...
[実装フェーズ 続き]
  ...
  ⌛ #8  設定 WebUI(/admin) — 設計詰め中(Issue #79)
...
### #8 設定 WebUI(`/admin`)  ⌛(Issue #79 で設計詰め中)
- サーバ側設定(`config.json`)の編集 UI
- クライアント側設定(`localStorage`)とは厳格に分離(`docs/architecture.md` §3.5)
- 認証モデル / 編集対象 / 反映タイミング / バリデーション / UI 構成 を Issue #79 §1-§6 で合意中
