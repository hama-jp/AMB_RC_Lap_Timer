...
> Status: **alpha**(実装フェーズ #1-#6 完了。Field Test α 自走分の初回実行準備中。リリース前)
...
- ✅ **実装フェーズ #1-#6**(完了): 「**採取先行**」方針で、実機データを根拠に各層を実装
...
- 🚧 **Field Test 準備**(ほぼ完了)
  ...
  - ⌛ Windows 自走 harness `scripts/fieldtest-*.ps1`(PR #72 マージ待ち)
- ⏳ **次のアクション**: ★ Field Test α
  - 自走分(Smoke / Replay round-trip / ZIP shape / USB pathshift / Soak)を Windows エージェントが `scripts\fieldtest-runall.ps1` で実行(#73)
  - 人手分(iOS Safari Speech / Sleep-Wake / 物理 USB / SmartScreen / mDNS)は現地セッション
- ⏳ そのあと: 7. replay モード → 8. 設定 WebUI → ★ Field Test β → 9. リリース自動化(`v0.1.0`)
