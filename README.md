...
> Status: **alpha**(実装フェーズ #1-#7 完了 + Field Test α-1 自走分実施済み。次は #8 設定 WebUI と Field Test α 人手分。リリース前)
...
- ✅ **実装フェーズ #1-#7**(完了): 「**採取先行**」方針で、実機データを根拠に各層を実装
  ...
  - ✅ 4. SPA 骨格(PR #60-#64)
  - ✅ 5. ラップ計算と表示(PR #66)
  - ✅ 6. 音声読み上げ + iOS Safari unlock overlay(PR #68)
  - ✅ 7. replay モード正式化(`fast = realtime × 10` + `--replay-speed` CLI、PR #77)
- ✅ **Field Test 準備**(完了)
  - `docs/field-test-log.md` 骨格(PR #69)
  - `tools/fieldtest/{tcp-emitter, ws-recorder, soak-monitor}`(PR #71)
  - Windows 自走 harness `scripts/fieldtest-*.ps1` 6 本(PR #72)
  - `packaging/README.txt` + `build.ps1` bundle(PR #78)
- ✅ **Field Test α-1**(自走分、PR #75): Win 11 で 5 シナリオすべて PASS(fan-out CV=0%、ws_mb +2.3%、handles +1.0%、reconnects=0)
- ⏳ **次のアクション**:
  - **8. 設定 WebUI(/admin)** — 設計詰め中(Issue #79 で §1-§6 合意)→ 実装 Issue 3 件に分割予定
  - **★ Field Test α 人手分** — iOS Safari Speech / Sleep-Wake / 物理 USB / SmartScreen / mDNS は現地セッションで実施
  - そのあと: ★ Field Test β → 9. リリース自動化(`v0.1.0`)
