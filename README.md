...
> Status: **alpha**(実装フェーズ #1-#8 + Field Test β-1 自宅 dry-run 完了。残るは #100 Sleep/Wake 修正 + β-2 現地(AMB 疎通)+ #9 リリース自動化)
...
| [`docs/field-test-log.md`](docs/field-test-log.md) | Field Test(実 LAN)の実施記録とテンプレート |
| [`docs/field-test-checklist-beta2.md`](docs/field-test-checklist-beta2.md) | β-2 現地(AMB 疎通)当日チェックリスト(印刷想定) |
| [`docs/roadmap.md`](docs/roadmap.md) | 実装フェーズの順序と各フェーズの責務(採取先行方針) |
| [`docs/captured-sessions/`](docs/captured-sessions/) | 実 LAN 現地データ採取セッションの観察メモ |
| [`docs/incidents/`](docs/incidents/) | 採取 / 開発中の incident 記録(再発防止用) |

---

## 開発状況とロードマップ

- ✅ **準備フェーズ**(完了): 仕様書整備、開発運用規約、バックログ Issue 起票
- ✅ **実装フェーズ #1-#8**(完了): 「**採取先行**」方針で、実機データを根拠に各層を実装
  - ✅ 1. `gateway-recorder` MVP(PR #39)
  - ✅ ★ 実 LAN 現地データ採取セッション(2026-05-05、`gateway/testdata/captured/session-2026-05-05.bin`)
  - ✅ 2. P3 パーサ TS 実装(PR #51)
  - ✅ 3. `gateway-full`(WS fan-out + `go:embed` + `/healthz` + `--replay`、PR #53)
  - ✅ 4. SPA 骨格(PR #60-#64)
  - ✅ 5. ラップ計算 + ベストラップ大型表示(PR #66 / #87)
  - ✅ 6. 音声読み上げ + iOS Safari unlock overlay(PR #68)+ digit-by-digit 形式(PR #103)
  - ✅ 7. replay モード正式化(`fast = realtime × 10` + `--replay-speed` CLI、PR #77)
  - ✅ 8. 設定 WebUI(`/admin`):認証(PR #88)+ API(PR #92)+ UI(PR #94)+ constant-time(PR #93)+ USB 移植性 fix(PR #105)
- ✅ **Field Test 準備**(完了): `tools/fieldtest/*`(PR #71)、Windows 自走 harness(PR #72)、`packaging/README.txt`(PR #78)、Firewall 復旧手順 docs(PR #104)、mock マルチ ponder(PR #91)
- ✅ **Field Test α-1**(自走分、PR #75): 5 シナリオすべて PASS
- ✅ **Field Test β-1**(自宅 dry-run、PR #96 + #102 + #106): Soak 1h + 9 シナリオ実施。派生 #98 / #99 / #101 はすべて修正、再検証で ✅ 化
- ⏳ **次のアクション**:
  - **#100 iPhone Sleep/Wake 後の再接続修正** — β-2 現地の唯一のブロッカー
  - **★ Field Test β-2 現地**(会場の AMB に WiFi で接続して疎通確認のみ、~30 分)
